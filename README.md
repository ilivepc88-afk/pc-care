...
- 体检 Windows、CPU、内存、系统盘、磁盘介质、连续运行时间和待重启状态；磁盘介质使用 Windows 原生只读存储查询，不引入 WMI 依赖。
...
2. 点击“开始体检”查看系统信息、启动项和后台优化状态；也可在对应页面单独扫描。
3. 如需修改当前用户项目，确认后直接生效；修改 HKLM、公共启动文件夹或第三方计划任务时，程序仅为该次固定操作请求 UAC。
4. 点击“导出报告”生成 HTML 和 JSON 文件。
5. 在“性能优化”页点击“一键应用性能模式”。该操作只修改当前 Windows 用户的视觉效果设置，不创建备份。
6. 在“后台优化”页可一键应用低风险推荐项，或逐项恢复为未配置默认状态。已有企业策略会被锁定，不会覆盖。
